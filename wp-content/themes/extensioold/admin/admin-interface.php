...
function siteoptions_add_admin() {
    ...
    $tt_page = add_menu_page($themename.' Options', $themename, 'edit_theme_options', 'siteoptions','siteoptions_options_page');
	add_action("admin_print_scripts-$tt_page", 'of_load_only');
	add_action("admin_print_styles-$tt_page",'of_style_only');
	//add_menu_page($themename." Options", "".$themename." Options", 'edit_themes', basename(__FILE__), 'siteoptions_add_admin');
} 
...
function siteoptions_machine($options) {
	...
	foreach ($options as $value) {
		...
		$output .= theme_is_pagetemplate_portfolio('portfolio%');
		break;		
		
		case "blog_add_cats":
		$output .= theme_is_pagetemplate_blog('blog%');
		break;		
