	...
	if ($slider_name == 'OneByOne Slider') {
		...
		for ($i=1;$i<=$slider_items_count;$i++) {
			if ( $slider_title[$i] || $slider_subtitle($i) || $slider_description[$i] || $slider_website_url($i) ) {
				...
				if ($slider_website_url[$i]) $slide_output .= '<span class="slidebutton'.$slider_text_align_class.'"><a href="'.$slider_website_url[$i].'"  '.$slider_button_colors_checkbox_style.' target="_blank">'.$slider_button_text[$i].'</a></span>';
