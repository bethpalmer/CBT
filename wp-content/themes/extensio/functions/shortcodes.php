...
function theme_column($atts, $content = null, $shortcodename = '') {
	...
	$title_url = ($title_url) ? '<a href="'.$title_url.'" class="homepagelink">' : '';
	$title_url_end = ($title_url) ? '</a>' : ''; 

	$title = ($title) ? '<'.$titlesize.'>'.$title_url.$title.$title_url_end.'</'.$titlesize.'>' : '';
...
function theme_latestposts2($atts, $content=null){
	...
	if ($wp_query->have_posts()) : while($wp_query->have_posts()) : $wp_query->the_post();					
		...
		$blog_items_output .= '
			<article class="post"'.$article_post_class.'>
				<figure class="visual"><a href="'.get_permalink().'">'.$blog_image_thumb.'</a></figure>
				<h3><a href="'.get_permalink().'">'.get_the_title().'</a></h3>
				<p><a href="'.get_permalink().'">'.__('Read More ...','extensio').'</a></p>
			</article>
		';
...
//[counsellingthumb name="Dr Emma Gray" pageurl="http://www.thebritishcbtcounsellingservice.com/dr-emma-gray/" pageimage="http://www.thebritishcbtcounsellingservice.com/wp-content/uploads/2015/05/Dr-Gray-Picture-sm.jpg" qualifications="DClinPsych, BA(Hons)" position="Consultant Clinical Psychologist and Service Director"][/counsellingthumb]
function theme_counsellingthumb($atts, $content=null){
	extract(shortcode_atts( array(
		"name" => "",
		"pageurl" => "",
		"pageimage" => "",
	...
	return '
		<article class="col-14 info-item team-item">
			<div>
				<p>
					<a href="'.$pageurl.'">
						<img class="boxshadow size-full wp-image-1426" title="'.$name.'" src="'.$pageimage.'" alt="'.$name.'" width="160" />
					</a>
				</p>
				<h2 class="team"><a style="color: #222;" href="'.$pageurl.'">'.$name.'</a></h2>
				<p class="qualifications">'.$qualifications.'</p>
				<p style="min-height:36px;">'.$position.'</p>
				<a href="'.$pageurl.'" class="btn green" style="	-moz-border-radius:3px; -webkit-border-radius:3px; border-radius:3px;" target="_self"><span>More info</span></a>
			</div>
		</article>	
	';
}
add_shortcode('counsellingthumb', 'theme_counsellingthumb');
